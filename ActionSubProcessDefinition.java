...
public class ActionSubProcessDefinition extends ProcessDefinition {

    public ActionSubProcessDefinition(IFile file) {
        super(file);
    ...
    public Object getPropertyValue(Object id) {
        if (PROPERTY_ACCESS_TYPE.equals(id)) {
            return Localization.getString("ProcessDefinition.property.accessType.EmbeddedActionSubProcess");
        }
    ...
    public void validate(List<ValidationError> errors, IFile definitionFile) {
        ...
        if (startStates.size() == 1 && startStates.get(0).getLeavingTransitions().size() != 1) {
            errors.add(ValidationError.createLocalizedError(startStates.get(0), "actionsubprocess.embedded.startState.required1leavingtransition"));
        }
        List<EndState> endStates = getChildren(EndState.class);
        for (EndState endState : endStates) {
            errors.add(ValidationError.createLocalizedError(endState, "actiosubnprocess.embedded.endState.notAllowed"));
        }
    }
